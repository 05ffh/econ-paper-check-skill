...
>
> **建议**：如果论文原稿是 Word 文档，请优先上传 `.docx` 格式，解析更稳定，质检结论更准确。
>
...
提醒必须放在**质检开始之前**，不得等报告生成后才说。用户明确回复"继续"或类似意图后再进入解析步骤。若用户能提供 Word 版，等新文件上传后重新走一次流程。
...
- **若 `source_format=pdf`，agent 必须在继续后续步骤前向用户发送标准“PDF 支持提醒”**（参见开头 §1 “用户上传 PDF 时的沟通礼仪”）。提醒必须包含：(a) PDF 解析存在已知局限（公式打散 / 表格错位 / 扫描件不可用）；(b) 建议优先上传 Word；(c) 若仅有 PDF，将尽力解析但建议对照原稿复核表格/公式相关问题；(d) 询问用户是否继续。**不得省略，不得只在报告报尾提。**
- 若用户回复能提供 Word 版，你等他上传后重新从x第一步开始。
- 若用户确认继续用当前 PDF，进入“PDF 降级策略”：整体证据强度默认下调一档；`parse_warnings` 中提及的表格/公式类目标一律转灰色需人工确认；总体风险等级出现“高风险”时，必须复核证据是否来自可读文本而非 PDF 噪声，否则改判“中风险 + 需人工确认”。
